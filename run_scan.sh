...
MEM_INFO="./heap_dumps/ptj.info"
MEM_DUMP="./heap_dumps/ptj.dump"

SCANNER_INFO="./scanner_info/scanner15.info"
...
echo ""
result=$(prompt_save __matches.partial.out)
echo ""
...
if [ $result != "USE" ]; then
  ...
  ${SCANNER_DIR}/${BUILD_CRIT_SCRIPT} ./bcrit ./output/__write.out $FUNC_LIST
  if [ ! -e $FUNC_LIST ]; then
    exit
  fi
  vim $FUNC_LIST
